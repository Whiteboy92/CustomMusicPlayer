...
public static class PlaybackHelper
{
	...
	{
		var displayed = playlist.GetDisplayedPlaylist();
		var currentSelectedSong = playlist.SelectedIndex >= 0 && playlist.SelectedIndex < displayed.Count
			? displayed[playlist.SelectedIndex]
			: null;
	...
	public static void RestorePlaybackState(ISettingsService settings, PlaylistView playlist, PlayerControlsView player, Dispatcher dispatcher, Action<MusicFile> onSongRestored, int? playlistId)
	{
		...
		{
			var allVisibleSongs = playlist.GetAllVisibleSongs();
			if (allVisibleSongs.Contains(song))
			{
				playlist.ClearSearch();
				displayedIndex = playlist.GetDisplayedPlaylist().IndexOf(song);
				if (displayedIndex >= 0)
				{
					playlist.SelectedIndex = displayedIndex;
				}
		...
		var autoPlay = settings.GetAutoPlayOnStartup();
