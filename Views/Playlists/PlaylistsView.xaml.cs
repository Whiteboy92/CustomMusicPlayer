...
namespace MusicPlayer.Views.Playlists
{
    ...
    public partial class PlaylistsView
    {
        ...
        public async void LoadPlaylists()
        {
            ...
            // Get the count of all songs in the music folder
            int allSongsCount = await musicLoader.GetMusicFileCountAsync();
            
            // Create a list that includes the default queue at the top
            var displayList = new List<Models.Playlist>();
            
            // Get the default queue settings from the settings service
            var defaultQueueSettings = settings.GetPlaylistById(-1);
            
            // Add default queue as first item
